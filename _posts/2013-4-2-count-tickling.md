...
<div><ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>返回当前日期的序列号。序列号是 Excel 日期和时间计算使用的日期-时间代码。如果在输入函数前，单元格的格式为“常规”，Excel 会将单元格格式更改为“日期”。如果要查看序列号，则必须将单元格格式更改为“常规”或“数值”。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HP010342958.aspx</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>FIND 用于查找其他文本字符串 (within_text) 内的文本字符串 (find_text)，并从 within_text 的首字符开始返回 find_text 的起始位置编号。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HP005209089.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>计算区域中满足给定条件的单元格的个数。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HP005209029.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>将条件应用于跨多个区域的单元格，并计算符合所有条件的次数。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HA102753238.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	<li><div>使用该函数时，发现不能满足一些特定条件的查找</div></li></ul></li></ul></li>
	<li><div>SUMPRODUCT()</div>
		<ul>
	<li><div>方法：</div>
		<ul>
	<li><div>在给定的几组数组中，将数组间对应的元素相乘，并返回乘积之和。数组参数必须具有相同的维数。 否则，函数 SUMPRODUCT 将返回 #VALUE! 错误值 #REF!。 函数 SUMPRODUCT 将非数值型的数组元素作为 0 处理。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HA102752853.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	<li><div>SUMPRODUCT((条件1)*(条件2)*(条件3)* …(条件n))，统计同时满足条件1、条件2到条件n的记录的个数。更多更强大使用的方法参见：http://blog.sina.com.cn/s/blog_4d712530010176u9.html</div></li></ul></li></ul></li>
	<li><div>EOMONTH()</div>
		<ul>
	<li><div>方法：</div>
		<ul>
	<li><div>返回指定月份数之前或之后的月份的最后一天的日期，该日期采用 datetime 格式。EOMONTH 可用于计算处于月份最后一天的到期日期。http://office.microsoft.com/zh-cn/excel-help/HA102837740.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>以 1900-9999 范围的四位整数形式返回日期的年份。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HA102837739.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>检查某个值是否为错误，并且返回 TRUE 或 FALSE。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HA102838076.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>根据 return_type 值返回给定日期和年份的周数。周数指示该周在数字上属于一年中的何处。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HA102838299.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>如果指定条件的计算结果为 TRUE，IF 函数将返回某个值；如果该条件的计算结果为 FALSE，则返回另一个值。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HP010342586.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>返回列中的最小数值。忽略逻辑值和文本。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HA102839077.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	...
	<li><div>方法：</div>
		<ul>
	<li><div>将月份返回为从 1（1 月）到 12（12 月）的数字。更详细参见：http://office.microsoft.com/zh-cn/excel-help/HA102838119.aspx?CTT=1</div></li></ul></li>
	<li><div>文档中：</div>
		<ul>
	<li><div>获取当前月份</div></li></ul></li></ul></li></ul></li>
	<li><div>在制作过程中注意到的</div>
		<ul>
	<li><div>混合引用</div>
		<ul>
	<li><div>参见：http://www.kaozc.com/computer/beikao/2514.html，http://blog.csdn.net/tianyi_lee/article/details/1815954</div></li></ul></li>
	<li><div>条件格式</div>
		<ul>
	<li><div>当对某区域（或单元格）应用多个条件时，若条件之间不是互斥关系，则效果会出错</div></li>
	<li><div>特别要注意“公式”和“单元格数值”的区别</div></li></ul></li>
	<li><div>COUNTIFS 和 SUMPRODUCT</div>
		<ul>
	...
	<li><div>http://www.tangrenke.com/2011/08/06/sum-under-multiconditions-countifs-sumproduct-database-function/</div></li>
